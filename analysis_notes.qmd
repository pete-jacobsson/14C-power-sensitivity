...
write_csv(single_cals_curve_uncert_diag, "single_cals_curve_uncert_diag.csv")
```
...
**After some reflection, visualizing the interaction model!!!**
...
plot_model_predictions_ocs(offset_dir = "neg", hpd_area = "accuracy_95",
                           ...
                           xlab = "Measurement and curve error")
```
OK, these lines are far straighter than what we had without interactions. Not running that model!
...
```{r}
summary(stan_offset_curve_sigma_comb[[1]]) ##68pos
```
```{r}
...
### MCMC diagnostics
Why go into MCMC diags after looking at model results? Well if model results were a mess, no need to do MCMC diags. If, on the other hand MCMC diags are OK, model results still can be a mess.
```{r}
...
```
OK, I don't like those correlations between sigma_curve_uncert and the intercept. Also the correlation between offset_magnitude and intercept for the 95% models is becoming concerning.
...
### Write function to draw predictions
```{r}
...
```{r}
predict_singles_accuracy("pos", "68", 40, 10, 30)[1:10]
predict_singles_accuracy("neg", "68", -40, 10, 30)[1:10]
predict_singles_accuracy("pos", "95", 40, 10, 30)[1:10]
predict_singles_accuracy("neg", "95", -40, 10, 30)[1:10]
```

### Plot predictions against input data
Here I have the functions to plot predictions against simulation inputs. We want to plot the median and twenty plausible lines


```{r}
draw_other_param <- function (other_var){
  ## Get the secondary variables for the plotting functions
  
  if (!str_detect(other_var, "offset")) {  ##Invert here to work easy with the plotting function
    secondary_var <- sample(single_cals_w_curve_uncert$offset_magnitude, 
                               1)} else {
    secondary_var <- sample(single_cals_w_curve_uncert$sigma_curve_uncert,
                               1)                               
                               }
  secondary_var
}
...
```{r}
draw_other_param("offset")
draw_other_param("uncert")
draw_other_param("chupacabra")
```
...
```
Looks good, now for the function to plot the medians.
...
```
Over a month later, do the actual plotting
...
Moral of the story:
Good control over impacts of offset magnitude, but too few sims to make predictions on error/curve uncert worth it. Also, error/curve uncert predicts are very sensitive to input offset magnitudes.

Moving forward this does make the case for taking a break and re-thinking the approach. Once we get into WMDs and Seqs the volumes of simulations will remain lower than what we can generate for single calibrations. This gives me pause, as I cannot be sure that I will be able to get a good control over the design features. In any case, this is enough to donwgrade project priority. Which means that all that's left at the moment is to tie this off and say why cutting losses. Moving forward, will want to change to a more fundamental approach.

We know we neeed those controls, because, working over the entire range of variability, the probabilities distributed at the intersects with nominal accuracy are very spread - "we lose nominal accuracy between 10 and 40 years offset" is not good enough. 
...
OK, out of curiosity, what happens when we allow the interaction in? Will it change the look at the error estimate?
...
stan_offset_curve_sigma_comb_68_pos_interact <- stan_glm(
  accuracy ~ offset_magnitude + sigma_curve_uncert + 
    offset_magnitude * sigma_curve_uncert,
  data = stan_df, family = binomial(link = 'logit'))
...
stan_offset_curve_sigma_comb_95_pos_interact <- stan_glm(
  accuracy ~ offset_magnitude + sigma_curve_uncert + 
    offset_magnitude * sigma_curve_uncert,
  data = stan_df, family = binomial(link = 'logit'))
...
stan_offset_curve_sigma_comb_68_neg_interact <- stan_glm(
  accuracy ~ offset_magnitude + sigma_curve_uncert + 
    offset_magnitude * sigma_curve_uncert,
  data = stan_df, family = binomial(link = 'logit'))
...
stan_offset_curve_sigma_comb_95_neg_interact <- stan_glm(
  accuracy ~ offset_magnitude + sigma_curve_uncert + 
    offset_magnitude * sigma_curve_uncert,
  data = stan_df, family = binomial(link = 'logit'))


stan_offset_curve_sigma_comb_interact <- list(
  stan_offset_curve_sigma_comb_68_pos_interact,
  stan_offset_curve_sigma_comb_68_neg_interact,
  stan_offset_curve_sigma_comb_95_pos_interact,
  stan_offset_curve_sigma_comb_95_neg_interact
)

```

```{r}
write_rds(stan_offset_curve_sigma_comb_interact, 
          "stan_offset_curve_sigma_comb_interact.rds")
```

```{r}
summary(stan_offset_curve_sigma_comb_interact[[1]]) ##68pos
```
```{r}
summary(stan_offset_curve_sigma_comb_interact[[2]]) ##68neg
```

```{r}
summary(stan_offset_curve_sigma_comb_interact[[3]]) ##95pos
```


```{r}
summary(stan_offset_curve_sigma_comb_interact[[4]]) ##95pos
```

```{r}
stan_offset_curve_sigma_comb_interact <- read_rds(
  "stan_offset_curve_sigma_comb_interact.rds")

posterior_68_pos_interact <- as.array(stan_offset_curve_sigma_comb_interact[[1]])
posterior_68_neg_interact <- as.array(stan_offset_curve_sigma_comb_interact[[2]])
posterior_95_pos_interact <- as.array(stan_offset_curve_sigma_comb_interact[[3]])
posterior_95_neg_interact <- as.array(stan_offset_curve_sigma_comb_interact[[4]])
```

```{r}
color_scheme_set("blue")

mcmc_hist(posterior_68_pos_interact, pars = c("(Intercept)", 
                                              "offset_magnitude",
                                              "sigma_curve_uncert", 
                                              "offset_magnitude:sigma_curve_uncert"))
mcmc_hist(posterior_68_neg_interact, pars = c("(Intercept)", 
                                              "offset_magnitude",
                                              "sigma_curve_uncert", 
                                              "offset_magnitude:sigma_curve_uncert"))
mcmc_hist(posterior_95_pos_interact, pars = c("(Intercept)", 
                                              "offset_magnitude",
                                              "sigma_curve_uncert", 
                                              "offset_magnitude:sigma_curve_uncert"))
mcmc_hist(posterior_95_neg_interact, pars = c("(Intercept)", 
                                              "offset_magnitude",
                                              "sigma_curve_uncert", 
                                              "offset_magnitude:sigma_curve_uncert"))
```
...
```{r}
mcmc_parcoord(posterior_68_neg_interact)
mcmc_parcoord(posterior_68_pos_interact)
mcmc_parcoord(posterior_95_neg_interact)
mcmc_parcoord(posterior_95_pos_interact)
```

```{r}
mcmc_pairs(posterior_68_pos_interact)
mcmc_pairs(posterior_68_neg_interact)
mcmc_pairs(posterior_95_pos_interact)
mcmc_pairs(posterior_95_neg_interact)
```
Plenty strong correls. Should be wary xP

```{r}
mcmc_trace(posterior_68_pos_interact)
mcmc_trace(posterior_68_neg_interact)
mcmc_trace(posterior_95_pos_interact)
mcmc_trace(posterior_95_neg_interact)
```
Good mixes.
...
```{r}
plot_stan_over_sims(posterior_68_pos_interact, 
                    range_to_plot = c(0, 50), 
                    hpd_area = "accuracy_68", 
                    variable = "offset_magnitude", 
                    rounding = 2, 
...
```{r}
plot_stan_over_sims(posterior_68_neg_interact, 
                    range_to_plot = c(-50, 0), 
                    hpd_area = "accuracy_68", 
                    variable = "offset_magnitude", 
                    rounding = 2, 
...
```{r}
plot_stan_over_sims(posterior_95_pos_interact, 
                    range_to_plot = c(0, 50), 
                    hpd_area = "accuracy_95", 
                    variable = "offset_magnitude", 
                    rounding = 2, 
...
```{r}
plot_stan_over_sims(posterior_95_neg_interact, 
                    range_to_plot = c(-50, 0), 
                    hpd_area = "accuracy_95", 
                    variable = "offset_magnitude", 
                    rounding = 2, 
...
OK, absolute mess. No solutions here!
...
## Part 15 Testing the model.
Model testing come in two flavours. First, I'll get more data and see if the parameters are still the same. After that I'll get some more data with specific values and run predictions, to see if they match expectations.

Now, I do not feel like running everything through OxCal again, so in the first instance I'll try using the relevant R packages (after checking for comparability with the ugly thing that is!).

```{r}
calibrate(2450, 50)
```
Which gices ever so slight differences to Oxcals 2708 - 2627 (23.8%) and 2622 - 2358 (71.7%). OK, lets re-hash how the OxCal code worked...

This feels like "Return to the Temple of Doom" or something...
...
### OxCal function re-build
Having done the few experiments above, I now have a clearer picture of how to approach this beast. The old singles OxCal relied on copying batches of 2k results from OxCal into Excel. I don't want to do it this way anymore, so instead I'll try something different. I will try setting up a function to generate a single OxCal result, collect the results and then delete the excess files.

#### New Simulate OxCal and test
```{r}
...
#### Component function1: execute_simulation
