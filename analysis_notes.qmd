...
$$
P(x) = \frac{1}{1 + e^{-(A_0 + A_1 x_1 + A_2 x_2 +...+A_n A_{n+1}x_1x_2+...)}}
$$
...
Now that we have a model with promising looking parameters, lets visualize it. This is always a little bit of a headache once we get to multiple regression, especially as we want to plot a few visuals to see if our predictions actually match the data they originated from. To simplify the process, lets make it more complex first. lets put together some functions to help us.

```{r}
### visualize_singles_logistic <- function(){
## This function takes on a simulated single calibration data frame, a logistic model output, ranges of the three variable parameters, and information which parameter to plot
## Raturns graph with histogram of accurate results overlain with the logistic regression fit.

## set_up_predicts_tab
## create_predicts
## filter_sims_data
## plot_sims_w_logistic
...
```{r}
### OK, lets try to make the creation of the prediction table a little more flexible-  this will pay off once we get to WMDs
...
test <- set_up_predicts_tab(offset = c(0, 50), sigma = c(8, 32), curve_uncert = c(10, 20))
...
```{r}
execute_predictions <- function(pred_table, is_positive, hpd_area) {
  ### The function takes on a prediction table to fill, re-organizes the model results and applies a prediction
  ### Assumes that the input table is a prediction table and that the is_pos and hpd_area arguments are logical and c(68, 95) respectively.
  
  model_results <- single_cals_curve_uncert_regr %>%
    filter(str_detect(model, "os") & 
             str_detect(model, hpd_area) &
             is_pos == is_positive)
  
  model_results <- model_results$model_results[[1]]
  
  
  pred_table <- pred_table %>%
    mutate(
      ratio_accurate = predict(model_results, 
                                  newdata = pred_table,
                                  type = "response") 
  )
...
```{r}
test1 <- execute_predictions(test, is_positive = TRUE, hpd_area = "68")
```
...
plot_logistic <- function(pred_df, sims_df, xlab, ylab, acc_target) {
  plot <- ggplot(data = sims_df) +
    geom_hline(yintercept = acc_target, color = "grey70") +
...
```{r}
## Get the numbers to underpin drawing of curves. Courtesy of Allan Cameron, over here: https://stackoverflow.com/questions/65081329/multiple-logistic-regression-ggplot-with-groups
