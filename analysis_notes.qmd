...
single_cals_curve_uncert_mod %>%
  ...
  theme_classic()
```
So we might want to re-think this again. Options are going in straight with a simple offset magnitude and measurement uncertainty model. The drawback will be loss of curve information, which we also want to capture. I could also run more simulations - say increase the number of sims to 200k, thus enabling a switch back to the period-based model. This may be a more reasonable approach. 
